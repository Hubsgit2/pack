let coins = 500;
    ...
    function calculateSellPrice(player) {
        const rarity = cardRarity(player);
        switch(rarity) {
            case "Ultimate": return Math.floor(player.price * 0.75); // 75% of price
            case "Legendary": return Math.floor(player.price * 0.65); // 65% of price
            case "Elite": return Math.floor(player.price * 0.55); // 55% of price
            case "Rare": return Math.floor(player.price * 0.50); // 50% of price
            case "Common": return Math.floor(player.price * 0.40); // 40% of price
            default: return Math.floor(player.price * 0.50);
        }
    }
    ...
    function displayCollection(){
        ...
        if(collection.length === 0) {
            div.innerHTML = "<p style='text-align:center; color:#666;'>Your collection is empty. Buy packs or cards from the store!</p>";
            return;
        }
        collection.forEach((p,index)=>{
            const sellPrice = calculateSellPrice(p);
            ...
            card.innerHTML=`
                ...
                <div class="card-content">
                    ...
                    <p>${p.team}</p>
                    <p>⭐ ${p.rating}</p>
                    <button onclick="sellCard(${index})">Sell for ${sellPrice} coins</button>
                </div>
    ...
    function sellCard(index){
        ...
        coins+=sellPrice;
        alert(`Sold ${card.name} (${cardRarity(card)}) for ${sellPrice} coins!`);
        updateCoins();
        displayCollection();
    }
